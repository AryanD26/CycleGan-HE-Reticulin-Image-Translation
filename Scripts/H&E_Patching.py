...
# H&E_Patch.py
# ==============================================================================
# This script processes Whole Slide Images (WSIs) of H&E stains. It performs
...
# ==============================================================================
# --- Input Settings ---
# Directory containing the raw H&E Whole Slide Images.
INPUT_DIR = 'Images'
# Glob pattern to find the specific H&E files to process.
INPUT_PATTERN = '*H&E*.ome.tif' 
...
def create_tissue_mask(image_np):
    ...
    # Apply Otsu's method to automatically find the best threshold value.
...
def process_image(image_path):
    ...
    # Initialize or load existing XML
    ...
    try:
        ...
        he_image_np = tifffile.imread(image_path)
        if he_image_np.dtype != np.uint8:
            he_image_np = (he_image_np / 256).astype(np.uint8) if he_image_np.dtype == np.uint16 else he_image_np.astype(np.uint8)
        print(f"Image loaded. Shape: {he_image_np.shape}")
    except Exception as e:
        print(f"!!! FATAL ERROR loading {image_filename}: {e}")
        return 0

    # --- Create and Save Tissue Mask ---
    tissue_mask = create_tissue_mask(he_image_np)
    ...
    height, width, _ = he_image_np.shape
    saved_count = 0
    start_index = len(xml_root.findall("Patch")) # Continue numbering from existing patches
    ...
    for x, y in tqdm(coords_to_process, desc="Saving Patches"):
        ...
        if check_tissue_percentage(mask_patch, TISSUE_THRESHOLD_PERCENT):
            patch_np = he_image_np[y:y + PATCH_SIZE, x:x + PATCH_SIZE]
...
if __name__ == "__main__":
    print("--- Starting H&E WSI Patch Extraction ---")
    ...
    print(f"Found {len(input_files)} H&E image(s) to process.")
    
    total_patches = 0
    for image_file in input_files:
        total_patches += process_image(image_file)
    
    print(f"\n{'='*80}\nAll H&E processing complete. Total patches extracted in this run: {total_patches}\n{'='*80}")